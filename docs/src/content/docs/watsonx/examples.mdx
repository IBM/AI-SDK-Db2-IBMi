...
```sql
call watsonx.watsonx_logoutjob();
call watsonx.watsonx_setapikeyforjob('x');
call watsonx.watsonx_setprojectidforjob('y');

-- Should return Y
values watsonx.watsonx_ShouldGetNewToken();

-- Should return Y
values watsonx.watsonx_authenticate();
...
```sql
values watsonx.watsonx_generate('Hello world');
values watsonx.watsonx_generate('Hello world', model_id => 'ibm/granite-3-8b-instruct');
```
...
```sql
values watsonx.watsonx_parameters(temperature => 0.5, time_limit => 1000);
values watsonx.watsonx_generate('Hello world', parameters => watsonx.parameters(temperature => 0.5, time_limit => 1000));
```
...
```sql
call watsonx.watsonx_GetModels();
```
